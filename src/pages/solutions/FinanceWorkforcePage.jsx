...
export function FinanceWorkforcePage() {
  const { navigate } = useRouter();
  const data = WORKFORCE_SOLUTIONS.find(s => s.id === 'finance') || WORKFORCE_SOLUTIONS[3];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12 text-left">
          <div className="p-6 rounded-xl bg-[#0D1B2A] border border-rose-900/50 shadow-xl">
            <span className="text-xs font-bold uppercase tracking-wider text-rose-400 block mb-2 font-mono">
              Financial Operation Bottleneck
            </span>
          ...
          <div className="p-6 rounded-xl bg-[#0D1B2A] border border-teal-500/40 shadow-xl">
            ...
            <p className="text-sm text-slate-300 leading-relaxed">
              Coordinated accounting agents automate OCR table extraction, 3-way PO matching, bank reconciliation diffing, and expense policy verification, while enforcing strict financial thresholds.
            </p>
          </div>
        </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <span className="text-xs font-bold uppercase tracking-widest text-amber-400 font-mono">
                Financial Compliance & Audit Control
              </span>
              <h3 className="text-xl font-bold text-white mt-1 mb-2">
                Mandatory Human Authorization Gates
              </h3>
              <p className="text-sm text-amber-200/90 leading-relaxed mb-4">
                To guarantee fiscal compliance and avoid unauthorized disbursements, the following actions require explicit controller or finance manager approval:
              </p>
        ...
        <div className="bg-[#0D1B2A] rounded-2xl border border-[#1E3553] p-8 shadow-2xl mb-12 text-left">
          <div className="mb-6">
            <span className="text-xs font-bold uppercase tracking-wider text-teal-400 font-mono block mb-1">
              Audit-Ready Sequence
            </span>
            <h3 className="text-2xl font-bold text-white">
              10-Stage Accounts Payable & Reconciliation Workflow
            </h3>
          </div>
        ...
        <div className="bg-[#0D1B2A] rounded-2xl border border-[#1E3553] p-8 shadow-2xl mb-12 text-left">
          <div className="mb-6">
            ...
            <h3 className="text-2xl font-bold text-white">
              8 Specialized Finance & Accounting Agents
            </h3>
          </div>
          ...
          <div className="lg:col-span-6 bg-[#0D1B2A] text-white p-6 rounded-2xl border border-[#1E3553] shadow-xl flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              <h4 className="text-base font-bold text-white mb-1">
                Explore an AP / Invoice POC
              </h4>
              <p className="text-xs text-slate-300">
                Validate document extraction and 3-way matching on sample vendor invoices.
              </p>
            </div>
            <Button
              variant="primary"
